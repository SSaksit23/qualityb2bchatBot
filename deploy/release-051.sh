...
release=/opt/qualityb2b-bobo-release-051
backup=/opt/qualityb2b-bobo/rollback-051
test ! -e "$release"
test ! -e "$backup"
mkdir -m 755 "$release"
tar -xzf "$stage/workflow-051.tar.gz" -C "$release"
cd "$release"
...
set +a
runuser -u qualityb2b-bobo -- node "$release/deploy/check-router-051.mjs"
mkdir -m 700 "$backup"
files='app.mjs booking.mjs live-booking.mjs product-search.mjs product-format.mjs workflow.mjs hermes.mjs package.json package-lock.json test.mjs workflow.test.mjs OPERATIONS.md IMPLEMENTATION_STATUS.md'
...
install -m 644 "$release/deploy/check-router-051.mjs" /opt/qualityb2b-bobo/deploy/check-router-051.mjs
systemctl start qualityb2b-bobo
sleep 2
