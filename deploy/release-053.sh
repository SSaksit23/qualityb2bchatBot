...
release=/opt/qualityb2b-bobo-release-053
backup=/opt/qualityb2b-bobo/rollback-053
test ! -e "$release"
test ! -e "$backup"
mkdir -m 755 "$release"
tar -xzf "$stage/workflow-053.tar.gz" -C "$release"
cd "$release"
...
set +a
runuser -u qualityb2b-bobo -- node "$release/deploy/check-router-053.mjs"
mkdir -m 700 "$backup"
...
install -m 644 "$release/deploy/check-router-053.mjs" /opt/qualityb2b-bobo/deploy/check-router-053.mjs
systemctl start qualityb2b-bobo
sleep 2
