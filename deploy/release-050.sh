...
release=/opt/qualityb2b-bobo-release-050
backup=/opt/qualityb2b-bobo/rollback-050
test ! -e "$release"
test ! -e "$backup"
mkdir -m 755 "$release"
tar -xzf "$stage/workflow-050.tar.gz" -C "$release"
cd "$release"
...
set +a
runuser -u qualityb2b-bobo -- node "$release/deploy/check-workflow.mjs"
mkdir -m 700 "$backup"
...
echo
trap - ERR
echo RELEASE_050_COMPLETE_WORKFLOW_ENABLED
