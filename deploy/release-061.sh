...
release=/opt/qualityb2b-bobo-release-061
backup=/opt/qualityb2b-bobo/rollback-061
test ! -e "$release"
test ! -e "$backup"
mkdir -m 755 "$release"
tar -xzf "$stage/workflow-061.tar.gz" -C "$release"
cd "$release"
...
for _ in $(seq 1 90); do
  health=$(curl -fsS http://127.0.0.1:3212/health || true)
  ready=$(printf '%s' "$health" | node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{try{process.stdout.write(String(JSON.parse(s).catalogReady===true))}catch{process.stdout.write('false')}})")
